...
def handle_command(command, channel, user):
    ...
    #quest
    if command.startswith("!hq") and channel == 'C5KJ7LHMX':
        command = command.split("!hq")[1].strip()
        response = HusciiQuest.hq(command, channel, user)
    else:
        if command.startswith("!hq"):
            response = '`Huscii quest can only be played in the #husciiquest channel`'
        ...
        for output in output_list:
            if output and 'text' in output and AT_BOT in output['text']:
                ...
                return output['text'].split(AT_BOT)[1].strip(), \
                       output['channel'], \
                       output['user']
            if output and 'text' in output and '!hq' in output['text']:
                return output['text'], \
                       output['channel'], \
                       output['user']
