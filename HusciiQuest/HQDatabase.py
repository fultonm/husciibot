...
class UserDB:
    ...
    def prof(self):
        ...
        response += '      | %-39s|\n' % ("Exp: " + str(self.getExpCur()) + "/" + str(self.getExpMax()))
        response += '      | %-39s|\n' % ("Combat Rating: " + str(self.getCR()))
...
class ShopDB:
    ...
    def sell(self, item):
        ...
        profile = self.profile
        if(len(item) != 0):
            item = UtilDB.itemDict(item)
            gold = self.getGold() + item['value']
            usercur.execute("UPDATE Profile SET Gold = ?", [gold])
            usercur.execute("DELETE FROM Inventory WHERE Item = ?", [item])
        else:
            response = "Can't find item"